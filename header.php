<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="utf-8">
	<meta http-equiv="X-UA-Compatible" content="IE=edge">
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<title>Crafts and Hobbies</title>
	<link href="https://fonts.googleapis.com/css?family=Montserrat:100,100i,200,200i,300,300i,400,400i,500,500i,600,600i,700,700i,800,800i,900,900i" rel="stylesheet">
	<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css" />
	<link rel="stylesheet" href="https://fonts.googleapis.com/icon?family=Material+Icons" />
	<link rel="stylesheet" href="css/bootstrap.min.css">
	<link rel="stylesheet" href="css/slick.css">
	<link rel="stylesheet" href="css/propeller.min.css">

	<link rel="stylesheet" href="css/select2.min.css">
	<link rel="stylesheet" href="css/select2-bootstrap.css">
	<link rel="stylesheet" href="css/pmd-select2.css">

	<link rel="stylesheet" href="css/slick-theme.css">
	<link rel="stylesheet" href="css/lightbox.min.css">
	<link rel="stylesheet" href="css/app.css">
</head>
<body>
	<header>
		<div class="container">
			<nav class="navbar navbar-default">
				<div class="container-fluid">
				<!-- Brand and toggle get grouped for better mobile display -->
				<div class="navbar-header">
					<button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#bs-example-navbar-collapse-1" aria-expanded="false">
						<span class="sr-only">Toggle navigation</span>
						<span class="icon-bar"></span>
						<span class="icon-bar"></span>
						<span class="icon-bar"></span>
					</button>
					<a class="navbar-brand" href="index.php">Brand</a>
				</div>
				<div class="rightHeader">
					<ul>
						<li class="menuDD">
							<a href="#"><i class="fa fa-user"></i></a>
							<ul class="menuDDlist">
								<li><a href="my-account.php">My account</a></li>
								<li><a href="my-orders.php">My orders</a></li>
								<li><a href="logout.php">Logout</a></li>
							</ul>
						</li>
						<li><a href="./cart.php"><i class="fa fa-shopping-cart"></i></a></li>
					</ul>
				</div>
				</div><!-- /.container-fluid -->
			</nav>
		</div>
		<div class="menuRow">
			<div class="container">
				<div class="row posRelative">
					<div class="col-sm-12">
						<!-- Collect the nav links, forms, and other content for toggling -->
					    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
							<ul class="nav navbar-nav">
								<!-- <li class="active"><a href="#">Link</a></li> -->
								<li class="dropdown allCats">
									<a href="#" class="dropdown-toggle" >All Categories <span class="caret"></span></a>
									<div class="dropdown-menu">
										<div class="allCatsList">
											<ul>
												<li>
													<a href="listing.php">Automotive</a>
													<div class="catsDD">
														<div class="col-sm-4">
															<ul>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
															</ul>
														</div>
														<div class="col-sm-4">
															<ul>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
															</ul>
														</div>
														<div class="col-sm-4">
															<ul>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
															</ul>
														</div>
													</div>
												</li>
												<li>
													<a href="listing.php">Electronincs</a>
													<div class="catsDD">
														<div class="col-sm-4">
															<ul>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
															</ul>
														</div>
														<div class="col-sm-4">
															<ul>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
															</ul>
														</div>
														<div class="col-sm-4">
															<ul>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
															</ul>
														</div>
													</div>
												</li>
												<li>
													<a href="listing.php">Jewelery</a>
													<div class="catsDD">
														<div class="col-sm-4">
															<ul>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
															</ul>
														</div>
														<div class="col-sm-4">
															<ul>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
															</ul>
														</div>
														<div class="col-sm-4">
															<ul>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
															</ul>
														</div>
													</div>
												</li>
												<li>
													<a href="listing.php">Kids & Babies</a>
													<div class="catsDD">
														<div class="col-sm-4">
															<ul>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
															</ul>
														</div>
														<div class="col-sm-4">
															<ul>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
															</ul>
														</div>
														<div class="col-sm-4">
															<ul>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
															</ul>
														</div>
													</div>
												</li>
												<li>
													<a href="listing.php">Watches</a>
													<div class="catsDD">
														<div class="col-sm-4">
															<ul>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
															</ul>
														</div>
														<div class="col-sm-4">
															<ul>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
															</ul>
														</div>
														<div class="col-sm-4">
															<ul>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
															</ul>
														</div>
													</div>
												</li>
												<li>
													<a href="listing.php">Clothing</a>
													<div class="catsDD">
														<div class="col-sm-4">
															<ul>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
															</ul>
														</div>
														<div class="col-sm-4">
															<ul>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
															</ul>
														</div>
														<div class="col-sm-4">
															<ul>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
																<li><a href="listing.php">Link 1</a></li>
															</ul>
														</div>
													</div>
												</li>
												<li><a href="listing.php">Bags</a></li>
											</ul>
										</div>
									</div>
								</li>
								<!-- <li class="dropdown">
									<a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">Dropdown</a>
									<div class="dropdown-menu">
										<div class="col-sm-4">
											<ul>
												<li><a href="#">Action 1</a></li>
												<li><a href="#">Another action 1</a></li>
												<li><a href="#">Something else here 1</a></li>
												<li role="separator" class="divider"></li>
												<li><a href="#">Separated link 1</a></li>
												<li role="separator" class="divider"></li>
												<li><a href="#">One more separated link 1</a></li>
											</ul>
										</div>
										<div class="col-sm-4">
											<ul>
												<li><a href="#">Action 1</a></li>
												<li><a href="#">Another action 1</a></li>
												<li><a href="#">Something else here 1</a></li>
												<li role="separator" class="divider"></li>
												<li><a href="#">Separated link 1</a></li>
												<li role="separator" class="divider"></li>
												<li><a href="#">One more separated link 1</a></li>
											</ul>
										</div>
										<div class="col-sm-4">
											<ul>
												<li><a href="#">Action 1</a></li>
												<li><a href="#">Another action 1</a></li>
												<li><a href="#">Something else here 1</a></li>
												<li role="separator" class="divider"></li>
												<li><a href="#">Separated link 1</a></li>
												<li role="separator" class="divider"></li>
												<li><a href="#">One more separated link 1</a></li>
											</ul>
										</div>
									</div>
								</li> -->
								<li class="active"><a href="#">My orders</a></li>
								<li class="active"><a href="#">Customer service</a></li>
							</ul>
					    </div><!-- /.navbar-collapse -->
					</div>
					<div class="col-xs-12">
						<div class="searchMainWrap">
							<div class="searchMain">
								<input type="text" id="searchMain" placeholder="Search here..." />
								<i class="fa fa-search"></i>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</header>
	<div class="clearfix"></div>
	<div class="pageWrapper"><!-- pageWrapper starts here -->